...
- **Link curto** (`/i/<codigo>`): cada link de anúncio ganha um código curto servido pelo próprio domínio, que redireciona (307) para a URL marcada. O código nasce da **origem** (`parceiro-x`, `instagram`), nunca da campanha — ele fica visível na barra de endereço, e o ângulo em teste não deve ser anunciado a quem clica. Único entre os não-nulos: dois links com o mesmo código creditariam a campanha de um ao outro. Colisão resolve por sufixo numérico; **nunca reaproveitando** o código de outro link.
  - O redirecionamento **não substitui a atribuição**: ele entrega o navegador na URL *com* os parâmetros, e é lá que o cookie de origem grava o primeiro toque. Redirect "limpo" apagaria a origem em silêncio.
  - Código desconhecido leva à landing, **não a 404**: erro de digitação no anúncio custa a atribuição, não deve custar também a visita. Pela mesma razão, API fora do ar redireciona para a landing em vez de mostrar erro.
  - 307 e não 308: um código pode ser reaproveitado para outra campanha, e redirecionamento permanente fica gravado no navegador de quem já clicou.
  - Encurtar é **conveniência de aparência, não parte da atribuição**: sem código, o link longo funciona igual. Por isso `slug` é anulável e nenhuma falha de encurtamento impede a criação do link.
