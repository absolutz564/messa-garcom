...
## Treinamento (10 min)
**Caixa** — abrir `/staff` num tablet/PC fixo e deixar aberto. **Clicar uma vez em "🔔 Som ligado"** ao abrir (o navegador só libera áudio depois de um clique); a partir daí toca uma campainha a cada solicitação e um toque curto a cada pedido, e repete a cada 15 s enquanto houver solicitação sem resposta. Volume do aparelho alto. No mesmo clique o navegador pergunta se permite **notificações**: aceitar — assim o balão aparece no canto da tela mesmo com o PDV ou outra aba na frente (requer o navegador aberto; se o navegador for fechado, não há alerta).
